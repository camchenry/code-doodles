...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords)
    {
        ...
        if (texture_coords.y + pH > love_ScreenSize.y) {
            Dpixel.a = 127;
        }
...
-- Smoke drawing = 
-- 1. draw smoke circles to first canvas
-- 2. copy smoke circles to second canvas
--
function love.draw()
    ...
    secondCanvas:renderTo(function()
        love.graphics.clear()
        love.graphics.setShader(Shader)
        love.graphics.draw(firstCanvas)
    end)

    firstCanvas:renderTo(function()
        love.graphics.setShader(Shader)
        love.graphics.clear()
        love.graphics.draw(secondCanvas)
...
function love.update(dt)
	...
	if(love.mouse.isDown(1))then 
		...
		--if mouse is up
		lastPoint.x, lastPoint.y = love.mouse.getPosition()
	end
end
