...
vec4 fractal_color_point(vec2 point_data) {
    ...
    vec3 hsv = vec3(hue + smoothing/5, saturation - smoothing/10, inSet ? value : 0);
    vec3 rgb = hsv2rgb(hsv);

    return vec4(rgb.rgb, smoothing*50);
}
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    for (int v = 0; v < supersampling; ++v) {
        for (int u = 0; u < supersampling; ++u) {
            ...
            vec4 color = fractal_color_point(fractal_julia(fx, fy, maxIterations));
            pixel_color += color;
        }
    }
